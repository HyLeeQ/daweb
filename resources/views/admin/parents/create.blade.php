    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-md-6 col-md-offset-3"> <!-- Center the form -->
                <form action="{{ route('admin.parents.store', ['user_id' => $user->id ?? null]) }}" method="POST"
                    class="fh5co-form">
                    @csrf
                    <!-- Ẩn ID của user -->
                    ...
                    <!-- Giáo viên - sử dụng Select -->
                    <div class="form-group">
                        <label for="student_teacher">Giáo Viên:</label>
                        <select name="student_teacher" class="form-control" required>
                            <option value="">Chọn giáo viên</option>
                            @foreach ($teachers as $teacher)
                                <option value="{{ $teacher->name }}">{{ $teacher->name }}</option>
                                <!-- Lấy tên giáo viên từ bảng teachers -->
                            @endforeach
                        </select>
                    </div>
